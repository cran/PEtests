...
simultest.cauchy <- function(dataX,dataY)
{
  X=dataX; Y=dataY
  ...
  stat_simu_cauchy = 0.5*tan((0.5-pval_mean_CQ)*pi) + 0.5*tan((0.5-pval_cov_LC)*pi)
  pval_simu_cauchy = 1-pcauchy(stat_simu_cauchy)
  return(list(stat = stat_simu_cauchy,
              pval = pval_simu_cauchy))
}
